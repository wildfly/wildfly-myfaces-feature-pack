...
# Prepare the command
command="mvn clean release:clean install release:prepare release:perform -Dmaven.repo.local=\"${LOCAL_REPO}\" -DdevelopmentVersion=\"${DEVEL_VERSION}\" -DreleaseVersion=\"${RELEASE_VERSION}\" -Dtag=\"${TAG_NAME}\" ${MAVEN_ARGS[*]}"
if [ "-v" = "${VERBOSE}" ]; then
    printf "\n\nExecuting:\n  "
    echo "${command}"
fi
...
if [ ${status} == 0 ]; then
    echo ""
    echo "Your release has been successful. Check the validation after 10 minutes on https://repository.jboss.org/nexus."
